...
const newer = require( 'gulp-newer' ); // newer files only
...
return  gulp
		.src( [ config.watchPhp ] )
		.pipe(gulp.dest(config.publicSRC))
		.pipe( notify({ message: '\n\n✅  ===> copy changed PHP to theme — completed!\n', onLast: true }) );
});
...
gulp.task(
	'default',
	gulp.parallel( 'styles', 'vendorsJS', 'customJS', 'images', 'php', 'translate', browsersync, () => {
		gulp.watch( config.watchPhp, gulp.series( 'php', reload ) ); // Reload on images changes.
		gulp.watch( config.watchStyles, gulp.parallel( 'styles' ) ); // Reload on SCSS file changes.
		gulp.watch( config.watchJsVendor, gulp.series( 'vendorsJS', reload ) ); // Reload on vendorsJS file changes.
		gulp.watch( config.watchJsCustom, gulp.series( 'customJS', reload ) ); // Reload on customJS file changes.
		gulp.watch( config.imgSRC, gulp.series( 'images', reload ) ); // Reload on images changes.
	})
);
...
// return  gulp
// 		.src( [ config.devSRC+'**' ], { allowEmpty: true } )
// 		.pipe(gulp.dest(config.publicSRC))
// 		.pipe( notify({ message: '\n\n✅  ===> copy — completed!\n', onLast: true }) );
// });

/* build */
// gulp.task("build", gulp.series('styles', 'vendorsJS', 'customJS', 'images', 'copyDev' ), () => {
// 	// return 	gulp
// 	// 	.src([ config.devSRC+'*.php' ] )
// 	// 	.pipe(gulpCopy(config.publicSRC))	// output path
// 	// 	.pipe(gulp.dest(config.publicSRC))
// 	// 	.pipe( notify({ message: '\n\n✅  ===> build — completed!\n', onLast: true }) );
// });
